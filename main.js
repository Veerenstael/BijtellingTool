...
// Zet de opmaak in de invoervelden
document.getElementById('cataloguswaarde').addEventListener('input', function() {
  let waarde = parseFloat(this.value.replace(/[^0-9.,-]+/g, ''));
  if (!isNaN(waarde)) {
    this.value = formatCurrency(waarde);
  }
});

document.getElementById('bruto-jaarinkomen').addEventListener('input', function() {
  let waarde = parseFloat(this.value.replace(/[^0-9.,-]+/g, ''));
  if (!isNaN(waarde)) {
    this.value = formatCurrency(waarde);
  }
